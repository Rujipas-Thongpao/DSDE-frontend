...
export default function Home() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setTickets([]);
    try {
      const res = await axios.post("http://localhost:3001/api/summarize", {
        text: textInput,
      });
    ...
    <div className="min-h-screen bg-white text-green-900 p-6">
      ...
      <form onSubmit={onSubmit}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
          <h1 className="text-xl font-bold text-green-1000">Traffy Summary</h1>
          <input
            name="text"
            value={textInput}
      ...
      <div id="result-summarize" className="mb-6">
        ...
        {summarize && (
          <h1 className="text font text-green-800">{summarize}</h1>
        )}
      </div>
